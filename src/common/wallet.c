...
static int parse_placeholder(buffer_t *in_buf, int version, policy_node_key_placeholder_t *out) {
    ...
    if (!buffer_read_u8(in_buf, (uint8_t *) &c) || c != '@') {
        return WITH_ERROR((int16_t) -1, "Expected key placeholder starting with '@'");
    }

    uint32_t k;
    if (parse_unsigned_decimal(in_buf, &k) == -1 || k > INT16_MAX) {
        return WITH_ERROR((int16_t) -1, "The key index in a placeholder must be at most 32767");
    }
    ...
    if (version == WALLET_POLICY_VERSION_V1) {
        ...
        if (!consume_character(in_buf, '/')           // the next character is "/"
            ...
            return WITH_ERROR((int16_t) -1, "Expected /** or /<M;N>/* in key placeholder");
        }
        ...
        if (next_character == '*') {
            ...
            if (out->num_first == out->num_second) {
                return WITH_ERROR((int16_t) -1, "M and N must be different in <M;N>/*");
            }
        ...
        return WITH_ERROR((int16_t) -1, "Invalid version number");
    }
